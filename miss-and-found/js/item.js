...
const personalityTheme = {
  ...
  curious: ["#4a7a8a", "#5e4a8a"],
  nostalgic: ["#8a6a6a", "#4a5a7a"],
  optimistic: ["#c47a4a", "#6a8a4a"]
};
...
const getPostImageUrl = (post) => {
  const loc = TRAVEL_LOCATIONS.find((l) => l.name === post.city);
  return loc?.image || "./images/placeholder-travel.jpg";
};
...
const postImageMarkup = (post) => {
  ...
  const imageAlt = `${post.city} 旅行风景`;
  const imageSrc = getPostImageUrl(post);
  const fallback = "./images/placeholder-travel.jpg";
  const onerror = `this.onerror=null;this.src='${fallback}';`;
  if (post.type === "encounter") {
    return `
      <div class="post-card__image">
        <img class="post-card__img-el" loading="lazy" src="${escapeHtml(imageSrc)}" onerror="${onerror}" alt="${escapeHtml(imageAlt)}" />
